    ...
    def after_epoch(self,meta):
        ...
        with open(self.csv_dir, 'w', newline='') as f:
            ...
            for i in range(len(meta['train_info']['train_loss'])):
                temp_data = [
                    i+1,
                    meta['train_info']['train_loss'][i],
                    meta['train_info']['val_acc'][i].get('accuracy_top-1'),
                    mean(meta['train_info']['val_acc'][i].get('precision',0.0)),
                    mean(meta['train_info']['val_acc'][i].get('recall',0.0)),
                    mean(meta['train_info']['val_acc'][i].get('f1_score',0.0))
                ]
        ...
        fig.legend()
        fig.tight_layout()
        plt.savefig(self.pic_dir, dpi=300)
        plt.close("all")
